...
parser.add_argument('--Micro_label', default='../Mix_label_23.txt')
parser.add_argument('--Micro_subject', default='../Mix_subject_23.txt')
parser.add_argument('--Micro_merge', default='../Mix_flow_23.txt')
...
def main():
    ...
    for epoch in range(args.Epoch):
        ...
        preds, target, loss, temp_file, samname, samtar, sampred, subs, filel = validate(test_loader, model,
                                                                                         criterion, epoch,
                                                                                         test_subjects)
        name.extend(samname)
        ...
        subss.extend(subs)
        filels.extend(filel)
        print(len(name), len(predss), len(targets), len(subss), len(filels))
        ...
        #            print('preds:{preds}'.format(preds=preds))
    ...
    file_handle.writelines('Runned Subject:\t'
                           'F1 Score: {cur_f1:.3f}\t'.format(
        cur_f1=current_f1) + '\n'
                           )
